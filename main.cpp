...
int main()
{
    for (auto i =1; i < 4; ++i)
    {
        std::cout << std::hex << M_add_rkey[i][0] << " " << RK_add_rkey[i][0] << " ";
        std::cout << std::hex << ((M_add_rkey[i][0] + RK_add_rkey[i][0])) << " " << C_add_rkey[i][0] << " " <<
                   (M_add_rkey[i][1] + RK_add_rkey[i][1]) << " " << C_add_rkey[i][1] << std::endl;
    }

    // RUN_BASIC_TRANSFORMATIONS_TESTS
