...
{
    uint8_t s_inv_test[4][256];

    s_key_inv(&s[0][0], &s_inv_test[0][0], 4, 256);

    if (memcmp(s_inv_test, s_inv, sizeof(uint8_t) * 4 * 256))
    {
        std::cerr << "s_key_inv_test FAILED!" << std::endl;
    }
    else
    {
        std::cerr << "s_key_inv_test PASSED!" << std::endl;
    }
}
