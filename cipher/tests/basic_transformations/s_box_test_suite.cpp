...
unsigned __int8 C1_s_box[10][16]={
...
unsigned __int8 M1_s_box[10][16]={
...
void run_s_box_test()
{
    ...
    for (auto i = 0; i < 10; ++i)
    {
        s_box(M1_s_box[i], &s[0][0], rows, columns);

        if (!compare_data_row((uint64_t*)M1_s_box[i], (uint64_t*)C1_s_box[i], columns))
        {
            failed = true;
        }
    }
    ...
    if (failed)
    {
        std::cerr << "s_box_test FAILED!" << std::endl;
    }
    else
    {
        std::cerr << "s_box_test PASSED!" << std::endl;
    }
}
